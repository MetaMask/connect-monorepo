...
/* eslint-disable @typescript-eslint/naming-convention -- Test naming and mock names */
...
function testSuite<T extends MultichainOptions>({
  ...
  t.describe(`${platform} tests`, () => {
    ...
    t.it(
      `${platform} should invoke method successfully from provider with an active session and connected transport`,
      async () => {
    ...
    t.it(
      ...
      async () => {
        ...
        mockedData.mockWalletInvokeMethod.mockRejectedValue(
          new Error('Failed to invoke method'),
        );
        ...
        await t
          .expect(sdk.invokeMethod(options))
          .rejects.toThrow(
            'RPCErr53: RPC Client invoke method reason (Failed to invoke method)',
          );
      },
      { timeout: 100000 },
    );
    ...
    t.it.skip(
      `${platform} should invoke readonly method successfully from client if infuraAPIKey exists`,
      async () => {
        const scopes = ['eip155:1'] as Scope[];
        const caipAccountIds = [
          'eip155:1:0x1234567890abcdef1234567890abcdef12345678',
