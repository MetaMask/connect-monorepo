...
t.describe('ModalFactory', () => {
  ...
  t.beforeEach(() => {
    ...
    const globalStub = {
      ...
      localStorage: {
        ...
        getItem: t.vi.fn(
          (key: string) => globalStub.localStorage.data.get(key) || null,
        ),
    ...
    const sessionStorageMock = {
      data: new Map<string, string>(),
      getItem: t.vi.fn(
        (key: string) => sessionStorageMock.data.get(key) || null,
      ),
      setItem: t.vi.fn((key: string, value: string) => {
    ...
    mockFactoryOptions = {
      InstallModal: t.vi
        .fn()
        .mockImplementation((options: InstallWidgetProps) => ({
          ...mockModal,
          options,
        })),
      OTPCodeModal: t.vi
        .fn()
        .mockImplementation((options: OTPCodeWidgetProps) => ({
          ...mockModal,
          options,
        })),
    } as any;
  ...
  t.describe('Constructor validation', () => {
    t.it('should throw an exception if required modals are not present', () => {
      t.expect(() => new ModalFactory({} as any)).toThrow(
        'Missing required modals: InstallModal, OTPCodeModal',
      );
    });
    ...
    t.it('should throw an exception if only some modals are missing', () => {
      ...
      t.expect(() => new ModalFactory(partialOptions as any)).toThrow(
        'Missing required modals: OTPCodeModal',
      );
    });
  ...
  t.describe('Modal lifecycle', () => {
    ...
    t.it(
      'should properly unmount previous modal when rendering new one',
      async () => {
        // Render first modal
        const connectionRequest: ConnectionRequest = {
          sessionRequest: {
  ...
  t.describe('Preload function error handling', () => {
    ...
    t.it(
      'should handle preload import failure gracefully and log error',
      async () => {
        ...
        t.vi.resetModules();
        const { preload: TestPreload } = await import('./preload');
        await TestPreload();
