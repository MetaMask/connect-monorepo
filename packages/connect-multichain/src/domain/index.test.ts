...
t.describe('Platform Detection', () => {
  ...
  t.describe('getPlatformType', () => {
    ...
    t.it(
      'should return MetaMaskMobileWebview when in MetaMask mobile webview',
      () => {
        t.vi.stubGlobal('window', {
          ReactNativeWebView: {},
          navigator: {
            userAgent:
              'Mozilla/5.0 (iPhone; CPU iPhone OS 15_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/15.0 Mobile/15E148 Safari/604.1 MetaMaskMobile',
          },
        });
        t.vi.stubGlobal('navigator', {
          userAgent:
    ...
    t.it(
      'should not return MetaMaskMobileWebview when ReactNativeWebView is missing',
      () => {
        t.vi.stubGlobal('window', {
          navigator: {
            userAgent:
    ...
    t.it(
      'should not return MetaMaskMobileWebview when userAgent does not end with MetaMaskMobile',
      () => {
        t.vi.stubGlobal('window', {
          ReactNativeWebView: {},
          navigator: {
...
t.describe('Logger', () => {
  ...
  t.describe('isEnabled', () => {
    t.it(
      'should return true when namespace is in process.env.DEBUG',
      async () => {
        process.env.DEBUG = 'metamask-sdk:core';

        const result = await isEnabled(
          'metamask-sdk:core',
          mockStoreClient as any,
        );
    ...
    t.it(
      'should return false when namespace is not in process.env.DEBUG',
      async () => {
...
t.describe('EventEmitter', () => {
  t.describe('emit', () => {
    t.describe('SDKEvents', () => {
      ...
      t.it(
        'should handle removing handlers from non-existent events gracefully',
        () => {
          const handler = t.vi.fn();
