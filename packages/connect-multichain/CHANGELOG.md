...
- Attach `error_code` and `error_message_sample` companion properties to `mmconnect_wallet_action_failed` and `mmconnect_connection_failed`. `error_code` preserves the raw wallet-side JSON-RPC / EIP-1193 code (e.g. `4001`, `-32603`) so the `unknown` `failure_reason` bucket stays triageable without a schema change every time a new code appears. `error_message_sample` is a sanitised, 200-char-max preview of the original error message — wallet addresses (EVM `0x{40}`, Solana / Bitcoin Base58 ≥32 chars, Bech32 with short HRPs like `bc1…` / `cosmos1…`), long hex blobs, URLs, and 10+-digit decimal numbers are scrubbed before emission. Both fields are optional and only set on the two `*_failed` events. The new `extractErrorDiagnostics` helper bundles all three diagnostic properties so producer call sites stay a single spread. Schema-side: [`metamask-sdk-analytics-api#32`](https://github.com/consensys-vertical-apps/metamask-sdk-analytics-api/pull/32). ([#293](https://github.com/MetaMask/connect-monorepo/pull/293))
