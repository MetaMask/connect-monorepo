...
function createStoreTests(
  ...
  t.describe(`${adapterName} error handling`, () => {
    ...
    t.it(
      'should throw StorageDeleteErr when removing a key fails',
      async () => {
...
t.describe(`Store with WebAdapter`, () => {
  ...
  t.it(
    "Should throw an exception if we try using the store with a browser that doesn't support localStorage",
    async () => {
      t.vi.stubGlobal('window', {
  ...
  createStoreTests(
    ...
    () => {
      t.vi
        ...
        .mockImplementation(async (key) => nativeStorageStub.getItem(key));
      t.vi
      ...
      t.vi
        ...
        .mockImplementation(async (key) => nativeStorageStub.removeItem(key));
    },
  );
});
