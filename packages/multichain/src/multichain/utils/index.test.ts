...
t.describe('Utils', () => {
  ...
  t.describe('getDappId', () => {
    ...
    t.it(
      'should return window.location.hostname if window and window.location are defined',
      () => {
        global.window = {
          location: {
    ...
    t.it(
      'should return dappMetadata.name if window is undefined and name is available',
      () => {
    ...
    t.it(
      'should return dappMetadata.url if window is undefined and name is not available but url is',
      () => {
        global.window = undefined as any;
    ...
    t.it(
      'should return "N/A" if window is undefined and neither name nor url is available',
      () => {
        global.window = undefined as any;
        const dappSettings = {};
  ...
  t.describe('setupInfuraProvider', () => {
    t.it(
      'should not set up infura provider if infuraAPIKey is not provided',
      async () => {
    ...
    t.it('should set up infura provider with infuraAPIKey', async () => {
      await utils.setupInfuraProvider(options);
      t.expect(options.api?.readonlyRPCMap?.['eip155:1']).toBe(
        `https://mainnet.infura.io/v3/testKey`,
      );
    });

    t.it(
      'Should allow customizing the readonlyRPCMap + merge with defaults',
      async () => {
        ...
        t.expect(options.api?.readonlyRPCMap?.[customChainId]).toBe(
          customEndpoint,
        );
      },
    );
  ...
  t.describe('setupDappMetadata', () => {
    ...
    t.it(
      'should set url to undenied if it does not start with http:// or https:// and favicon is undefined',
      async () => {
    ...
    t.it(
      'should prove that dapp is mandatory is platform is not browser',
      async () => {
    ...
    t.it(
      'should prove that dapp is optional is platform is browser',
      async () => {
