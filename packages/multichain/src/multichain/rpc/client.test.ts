    ...
    t.describe('redirect to provider cases', () => {
      ...
      t.it(
        ...
        async () => {
          ...
          await t
            .expect(rpcClient.invokeMethod(baseOptions))
      ...
      t.it(
        'should throw RPCHttpErr when fetch response is not ok',
        async () => {
          const mockResponse = {
            ok: false,
            status: 500,
      ...
      t.it(
        'should use only Infura URLs when readonlyRPCMapConfig is not provided',
        async () => {
          ...
          const clientWithoutRPCMap = new rpcClientModule(
            mockTransport,
            configWithoutRPCMap,
            sdkInfo,
          );
      ...
      t.it(
        'should redirect to provider for methods in METHODS_TO_REDIRECT',
        async () => {
      ...
      t.it(
        'should redirect to provider when no RPC endpoint is available',
        async () => {
      ...
      t.it(
        'should redirect to provider when no infura API key and no custom RPC map',
        async () => {
          const noConfigClient = new rpcClientModule(
            mockTransport,
            {} as any,
            sdkInfo,
          );
      ...
      t.it('should throw RPCInvokeMethodErr when provider throws', async () => {
        ...
        await t
          .expect(rpcClient.invokeMethod(redirectOptions))
