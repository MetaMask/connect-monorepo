...
export const setupRNMocks = (nativeStorageStub: NativeStorageStub) => {
  ...
  vi.spyOn(AsyncStorage, 'getItem').mockImplementation(async (key) =>
    nativeStorageStub.getItem(key),
  );
  vi.spyOn(AsyncStorage, 'setItem').mockImplementation(async (key, value) =>
    nativeStorageStub.setItem(key, value),
  );
  vi.spyOn(AsyncStorage, 'removeItem').mockImplementation(async (key) =>
    nativeStorageStub.removeItem(key),
  );
};
