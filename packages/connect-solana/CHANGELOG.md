...
- `createSolanaClient()` now eagerly initializes the Solana wallet provider during creation. If the underlying multichain session already contains solana scopes, the provider's accounts are populated before the client is returned. ([#277](https://github.com/MetaMask/connect-monorepo/pull/282))
- `getWallet()` now returns the same wallet instance on every call instead of constructing a new one. ([#277](https://github.com/MetaMask/connect-monorepo/pull/282))
