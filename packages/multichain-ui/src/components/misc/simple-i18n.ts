...
/* eslint-disable no-restricted-globals -- i18n uses navigator for locale detection */
/* eslint-disable id-length -- 't' is standard i18n function name */
type I18nInstance = {
  t: (key: string) => string;
...
const defaultTranslations: TranslationDict = {
  ...
  INSTALL_MODAL: {
    ...
    INSTALL_META_MASK_EXTENSION_TEXT:
      'Install and try the MetaMask browser extension.',
  },
  PENDING_MODAL: {
...
export class SimpleI18n implements I18nInstance {
  ...
  async loadTranslations(locale: string): Promise<void> {
    ...
    try {
      ...
      console.warn(
        `❌ Failed to load ${shortLocale} translations, falling back to English:`,
        error,
      );
      this.translations = defaultTranslations;
    }
