...
export default function SolanaScreen() {
  ...
  useEffect(() => {
    (async () => {
      ...
      c.core.on('wallet_sessionChanged', (session) => {
        const caipAccounts =
          session?.sessionScopes?.[MAINNET_SCOPE]?.accounts ?? [];
  ...
  const handleSignMessage = async () => {
    ...
    try {
      const message = Buffer.from('Hello from React Native!').toString(
        'base64',
      );
      const result = await client.core.invokeMethod({
        ...
        request: {
          method: 'signMessage',
          params: { account: { address: accounts[0] }, message },
        },
      });
      Alert.alert(
        'Signed',
        JSON.stringify(result.signature).slice(0, 40) + '...',
      );
    } catch (err: any) {
...
- **Call `createSolanaClient` once** — each call returns a _new_ Solana client wrapper, but they share the singleton multichain core (whose options merge across calls). Create it once (e.g., in a `useEffect` or before app registration), don't recreate per render.
