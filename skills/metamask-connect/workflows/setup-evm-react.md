...
- **Call `createEVMClient` once at app startup** — store the promise and reuse it; never call it per render. Each call returns a _new_ EVM client wrapper, but they all share one underlying multichain core (the core is the singleton, and its options are merged across calls).
