...
const metamaskOpenrpcDocument: OpenrpcDocument =
...
export function ScopeCard({
  ...
  scope: Scope;
  details: SessionData['sessionScopes'][Scope];
}) {
  ...
  const handleInvokeMethod = async (
    ...
    if (!requestObject && !scopeRequest) {
      throw new Error(
        `No request configured for method ${method} on scope ${scope}`,
      );
    }

    const finalRequestObject =
      requestObject ?? JSON.parse(scopeRequest ?? '{}');
      ...
      >
        ...
        <div className="p-4 bg-white border-t border-gray-200">
          ...
          <div className="mt-2 text-xs text-gray-500">
            Edit the JSON above to customize the request parameters before
            invoking the method.
          </div>
        </div>
      </details>
        ...
        ([method, results]) => {
          return results.map(({ result, request }, index) => {
            ...
            return truncated ? (
              ...
              >
                <summary
                  className={`px-4 py-3 cursor-pointer hover:bg-gray-50 transition-colors duration-150 rounded-t-lg ${
                    isError ? 'bg-red-50 border-red-200' : 'bg-gray-50'
                  }`}
                >
                      ...
                      <div className="flex items-center gap-2 mb-1">
                        <span
                          className={`font-mono text-sm font-medium ${
                            isError ? 'text-red-700' : 'text-purple-700'
                          }`}
                        >
                          {method}
                        </span>
