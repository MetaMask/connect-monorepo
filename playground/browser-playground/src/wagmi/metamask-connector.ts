    ...
    async function ensureMetamask() {
      if (!metamask) {
        if (!metamaskPromise) {
          ...
          const createOptions: Parameters<typeof createMetamaskConnectEVM>[0] = {
          ...
          if (parameters.debug !== undefined) {
            createOptions.debug = parameters.debug;
          }
          metamaskPromise = createMetamaskConnectEVM(createOptions);
        }
    ...
    const connector = {
      ...
      async switchChain(
        ...
        const chainConfiguration: {
          chainId: string;
          rpcUrls?: string[];
          nativeCurrency: typeof chain.nativeCurrency;
          chainName: string;
        ...
        if (rpcUrls) {
          chainConfiguration.rpcUrls = rpcUrls;
        }
        if (blockExplorerUrls) {
          chainConfiguration.blockExplorerUrls = blockExplorerUrls;
        }
        if (addEthereumChainParameter?.iconUrls) {
          chainConfiguration.iconUrls = addEthereumChainParameter.iconUrls;
        }
